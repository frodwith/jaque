...
(deftest bad-fragment
  (is (thrown+? [:type :jaque.error/bail :bail-type :exit]
                (nock empty-machine a0 (noun [0 0])))))

(defjet math-add [add math kmat] - [+<- +<+] [^Atom a ^Atom b]
  (.add a b))

(defjet math-sub [sub math kmat] - [+<- +<+] [^Atom a ^Atom b]
  (.sub a b))

(defjet math-dec [dec math kmat] - [+<] [^Atom a]
  (.sub a a1))

(defjet math-lth [lth math kmat] - [+<- +<+] [^Atom a ^Atom b]
...
(def m0 (assoc empty-machine :dash (-> empty-dashboard
                                       (.install-jet math-add)
                                       (.install-jet math-sub)
                                       (.install-jet math-dec)
                                       (.install-jet math-lth))))
(deftest math-kernel-test
  (let [[m1 ken] (nock m0 a0 math-kernel-formula)
        [m2 r]   (nock m1 ken (noun [8 [9 22 0 1] 9 2 [0 4] [1 10] 0 11]))]
    (is (= 55 r))))
